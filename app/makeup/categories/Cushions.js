...
const Cushions = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get("http://localhost:5004/cushions");
          ...
          {sortedProducts.map((product) => (
            ...
            >
              ...
              <div className="mt-2">
                ...
                {product.mrp !== null && product.sp === null && (
                  <p className="text-[#333333] font-bold text-xl">Lek {product.mrp}</p>
                )}
                {product.sp !== null && (
